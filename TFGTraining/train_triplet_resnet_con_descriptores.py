...
cityPath = "/Users/frederikwarburg/Desktop/Zaragoza/dataset/LondonFeatures/"
...
nLabels = len(data[:,0]) # length of data
...
plt.plot(np.linspace(0,len(loss),1),loss,'-o')
mean = np.convolve(np.array(loss), np.ones((N,))/N, mode='valid')
plt.plot(np.linspace(0,len(loss),len(mean)),mean,'r-o')
